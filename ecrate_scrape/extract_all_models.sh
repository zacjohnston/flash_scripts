...
if [ -z "${FLASH_MODELS}" ]; then
  echo "Must define path FLASH_MODELS"
  exit 1
fi
if [ -z "${FLASHBANG}" ]; then
  echo "Must define path FLASHBANG"
  exit 1
fi

script="${FLASHBANG}/scripts/extract_model.py"
config="stir"
threads=16
reload="True"

for tab in "${tabs[@]}"; do
  model_set="run_ecrates_tab${tab}"

  for mass in "${masses[@]}"; do
    model="run_${mass}"
    run="stir_ecrates_tab${tab}_s${mass}_alpha1.25"

#    dat_path="${FLASH_MODELS}/${model_set}/${model}/${run}.dat"
#    rm "${dat_path}"

    python "${script}" "${run}" "${model}" "${model_set}" reload="${reload}" config="${config}" threads="${threads}"
